...
| `PATH_NOT_ALLOWED` | no | no | A patch-file path was refused by path policy. Four causes, with different fixes: it fell **outside** the patch/checkpoint roots under the Rack user directory (use a path within the configured roots); it is in the **wrong root for the tool** — `save_patch` and `preview_load_patch` take the patches root, `restore_checkpoint` the checkpoints root (use the tool that owns that root); it is not a `.vcv`, is a reserved Windows device name, or is not a regular file; or the file **exists but is empty**, which is a write that never completed rather than a patch (a `.vcv` of 0 bytes cannot be loaded — Rack clears the patch before reading the archive, so loading one would empty the rack). A failure *writing* a checkpoint is `INTERNAL`, not this. |
