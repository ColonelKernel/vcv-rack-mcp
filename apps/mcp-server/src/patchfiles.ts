...
async function buildLoadPreview(
  ...
  const preview = {
    ...
    // the resulting patch lacks it) so it can reconnect after restart.
    willInsertBridgeModule: kind === "clear",
    risk: {
      level: "high" as const,
    ...
    warnings: currentSaved ? [] : ["the current patch has unsaved changes"],
  };
...
async function commitLoadOrClear(
  ...
  if (binding.kind !== kind || binding.instanceId !== instance.instanceId) {
    throw new ToolError("CONFIRMATION_REQUIRED", "confirmation token does not match this operation");
  }
  await ctx.conn.ensureLease();

  // Automatic recovery checkpoint before the destructive operation.
  const stampMs = Date.now();
  const recoveryPath = checkpointPath(config, "recovery", stampMs);
  let recoveryCheckpointPath: string | null = null;
  try {
    ...
    recoveryCheckpointPath = recoveryPath;
  } catch {
    recoveryCheckpointPath = null;
  }
  ...
  if (!args.confirmationToken) {
    ...
    return {
      ...
      preview: {
        ...
        warnings: status.saved === true ? [] : ["the current patch has unsaved changes"],
      },
      confirmation: {
        confirmationToken: token,
        confirmationExpiresAt: new Date(Date.now() + 5 * 60 * 1000).toISOString(),
